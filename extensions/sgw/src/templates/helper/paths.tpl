...
{/if}
  ...
  {if !$popup && !$print}
    ...
    {if !$sys.browser.is_mobile}
	  ...
	  <span class="cursor" onclick="nWin('index.php?print=1&print_all=1');">{t}Print all{/t}</span> |{" "}
	{/if}
    ...
    {if $sys.username neq "anonymous" && !$sys.is_superadmin}
	  ...
	  &nbsp;<a href="offline.php"><img src="ext/icons/offline.png" title="{t}Offline folders{/t}"/></a>{" "}
    {/if}
...
{if !$print && $t.views[$t.view].ENABLE_CALENDAR neq ""}
  ...
  <span class="default10 baseline">
	&nbsp;<span class="cursor" onclick="locate('index.php?today=now')">{t}Today{/t}</span>&nbsp;{" "}
    <input type="button" id="datebox_button" value="{$datebox.today|modify::localdateformat:"{t}M j, Y{/t}"|default:"..."}" onclick="calendar('datebox',false,true);">
	<input type="hidden" id="datebox" name="datebox" value="{$datebox.today|modify::dateformat:"{t}m/d/Y{/t}"}">&nbsp;{" "}

    <span class="cursor {if $datebox.mark eq "day"}bold{/if}" onclick="locate('index.php?markdate=day')">{t}Day{/t}</span>&nbsp;
	<span class="cursor {if $datebox.mark eq "week"}bold{/if}" onclick="locate('index.php?markdate=week')">{t}Week{/t}</span>{" "}
	<span class="cursor {if $datebox.mark eq "gantt"}bold{/if}" onclick="locate('index.php?markdate=gantt')">Gantt</span>&nbsp;
	<span class="cursor {if $datebox.mark eq "month"}bold{/if}" onclick="locate('index.php?markdate=month')">{t}Month{/t}</span>&nbsp;
	<span class="cursor {if $datebox.mark eq "all"}bold{/if}" onclick="locate('index.php?markdate=all')">{t}All{/t}</span>
  </span>
{/if}
