...
// +----------------------------------------------------------------------+
require_once 'lib/pear/Socket.php';
require("lib/mail/utf7.php");
...
class Net_IMAPProtocol {
    ...
    */
    ...
    function Net_IMAPProtocol()
    {
        ...
        if ((include_once 'lib/pear/SASL.php') == false) {
    ...
    function cmdAuthenticate($uid , $pwd , $userMethod = null)
    {
        ...
        switch ($method) {
            case 'DIGEST-MD5':
                $this->_authDigest_MD5( $uid , $pwd , $cmdid );
                break;
            case 'CRAM-MD5':
                $this->_authCRAM_MD5( $uid , $pwd ,$cmdid );
                break;
            case 'LOGIN':
                $this->_authLOGIN( $uid , $pwd , $cmdid );
                break;

            default :
                new PEAR_Error( "$method is not a supported authentication method" );
                break;
        }
   ...
   /*
    ...
    function cmdSort($fetchparam)
    {
        return $this->_genericCommand('SORT' , "$fetchparam" );
    }
    ...
    function cmdStatus($mailbox, $request)
    {
        ...
        if (!in_array($request,array("MESSAGES","RECENT","UIDNEXT","UIDVALIDITY","UNSEEN","HIGHESTMODSEQ","CLOSED"))) {
            // todo:  fix this error!
...
********************************************************************/
    ...
    */
    ...
    function _parseBodyResponse(&$str, $unused){
    ...
    * @since  1.0
    */
    function _parseContentresponse(&$str, $unused)
    {
        $content = '';
        $this->_parseSpace($str , __LINE__ , __FILE__ );
    ...
    */
    ...
    {
        if($printError){
            echo htmlspecialchars($line).",$file,PROTOCOL ERROR!:".htmlspecialchars($str)."\n";
        }
    }
...
//        case "RFC822.HEADER" :
        ...
        case "SORT" :
		    ...
		    break;
        case "STATUS" :
		 		// fix for gmail imap, replace " " in mailbox name with "_"
				if (($pos = strpos($str," ("))) {
				  $str = " ".str_replace(" ","_",substr($str,1,$pos-1)).substr($str,$pos);
				}
                $mailbox = $this->_parseOneStringResponse( $str,__LINE__ , __FILE__ );
                $this->_parseSpace( $str , __LINE__ , __FILE__ );
    ...
    */
        ...
        if( $space != ' ' and $space != ')' ){
            $this->_prot_error("2_must be a ' ' but is a '$space' !!!!" , $line , $file,$printError );
        }
        return $space;
    }
    ...
    */
        ...
        if( strtoupper($char_aux) != strtoupper( $char ) ){
            $this->_prot_error("must be a '$char' but is a '$char_aux' !!!!_", $line , $file ); 
        }
        return $char_aux;
    }
