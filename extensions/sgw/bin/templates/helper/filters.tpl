...
<table cellspacing="0" class="data noprint" style="border-top:0px; margin-bottom:2px;">
  ...
  <td style="vertical-align:top; width:90%;">
    ...
    {foreach item=filter from=$t.sqlfilters}
	  <div style="margin-bottom:2px;">
		...
		<select class="filter_type">
		  <option value="like" {if $filter.type eq "like"}selected{/if}> contains
		  <option value="nlike" {if $filter.type eq "nlike"}selected{/if}> not contains
		  <option value="starts" {if $filter.type eq "starts"}selected{/if}> starts with
		  <option value="eq" {if $filter.type eq "eq"}selected{/if}> equal
		  <option value="neq" {if $filter.type eq "neq"}selected{/if}> not equal
		  <option value="oneof" {if $filter.type eq "oneof"}selected{/if}> one of
		  <option value="lt" {if $filter.type eq "lt"}selected{/if}> lesser than
		  <option value="gt" {if $filter.type eq "gt"}selected{/if}> greater than
		</select>
	...
	{if $t.sqlfilters}
	  <input type="button" value="Clear" onclick="locate('index.php?folder='+escape(tfolder)+'&view='+tview+'&filters=');">
	{/if}
