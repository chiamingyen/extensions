...
{if $tree.type eq "folders" && !$onecategory}
  <td class="tabstyle2" onclick="locate('index.php?treetype=folders')">Folders</td>
  <td style="width:2px;"></td>
  <td class="tabstyle" onclick="locate('index.php?treetype=categories')">Categories</td>
{else}
  <td class="tabstyle" onclick="locate('index.php?treetype=folders')">Folders</td>
  <td style="width:2px;"></td>
  <td class="tabstyle2" onclick="locate('index.php?treetype=categories')">Categories</td>
{/if}
...
{if $datebox}
  ...
  <tr>
    ...
    <td style="width:45%;">
	  <input type="button" id="datebox_button" value="{$datebox.today|modify::localdateformat:"M j, Y"|default:"..."}" onclick="calendar('datebox',false,true);" style="margin-right:2px; margin-bottom:2px;">
	  <input type="hidden" id="datebox" name="datebox" value="{$datebox.today|modify::dateformat:"m/d/Y"}">
    </td>
	{/if}
	<td style="width:55%;">
	  <a href="#" onclick="locate('index.php?markdate=day'); return false;" {if $datebox.mark eq "day"}class="bold"{/if}>Day</a> &nbsp;
	  <a href="#" onclick="locate('index.php?markdate=week'); return false;" {if $datebox.mark eq "week"}class="bold"{/if}>Week</a> &nbsp;
	  <a href="#" onclick="locate('index.php?markdate=gantt'); return false;" {if $datebox.mark eq "gantt"}class="bold"{/if}>Gantt</a><br>
	  <a href="#" onclick="locate('index.php?markdate=month'); return false;" {if $datebox.mark eq "month"}class="bold"{/if}>Month</a> &nbsp;
	  <a href="#" onclick="locate('index.php?markdate=year'); return false;" {if $datebox.mark eq "year"}class="bold"{/if}>Year</a> &nbsp;
	  <a href="#" onclick="locate('index.php?markdate=all'); return false;" {if $datebox.mark eq "all"}class="bold"{/if}>All</a> &nbsp;
	  {if $datebox.mark neq "all"}<a href="#" onclick="locate('index.php?today=now'); return false;"><img title="Today" class="cursor" src="ext/icons/date.gif"></a>{/if}
	</td>
  </tr>
  </table>
{/if}
...
<tr><td class="search_bar">
  ...
  <table id="search_bar" class="tree2" border="0" cellpadding="0" cellspacing="0"><tr>
	<td style="width:100%; padding-left:3px;">
	  <input id="search_query" accesskey="f" onblur="searchbox2(this);" onfocus="searchbox(this);" style="width:100%;" type="text" value="{$t.search.query|default:"Search"}">
	</td>
	...
	{if $t.isdbfolder}
	  ...
	  <td class="default nowrap">Modified:</td>
	  <td></td>
	  <td>
	  <select id="search_modified">
		<option value="gt" {if $t.search.modified.type eq "gt"}selected{/if}> after
		<option value="lt" {if $t.search.modified.type eq "lt"}selected{/if}> before
	  </select>
	  </td><td>&nbsp;
	  ...
	  <td class="default">Subfolders:</td>
	  <td></td>
...
<tr><td class="tree_cpane" id="pane_options" onclick="tree_folder_options();">&nbsp;&gt; Options</td></tr>
{/if}
{if !$t.att.DISABLE_FOLDER_OPERATIONS && (($t.rights.write_folder && !$sys.mountpoint_admin) || $t.rights.admin_folder) && $t.isdbfolder}
<tr><td class="tree_cpane" id="pane_mountpoint" onclick="tree_folder_mountpoint();">&nbsp;&gt; Mountpoint</td></tr>
{/if}

<tr><td class="tree_cpane" onclick="tree_folder_info();">&nbsp;&gt; Info</td></tr>
