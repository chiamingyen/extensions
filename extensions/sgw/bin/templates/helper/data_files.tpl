...
<img src="ext/icons/download.gif" title="Download" style="margin-bottom:2px; margin-right:2px;"></a>&nbsp;
...
{if $item.locked[$key_filter] && !($t.rights.write && $item.can_unlock[$key_filter])}
  ...
  (Locked by {$item.data[$key_filter]|sys_get_lock})&nbsp;
{elseif $t.rights.write && $t.isdbfolder && !$iframe}
  {if $item.can_unlock[$key_filter]}
	...
	<img src="ext/icons/lock.gif" title="Unlock" style="margin-bottom:1px;"></a>&nbsp;
  {else}
    <a href="#" onclick="file_func('file_lock', '{$data_item._id}', '{$curr_id}', '{$key_filter}'); return false;">
	<img src="ext/icons/lock_open.gif" title="Lock" style="margin-bottom:1px;"></a>&nbsp;
  {/if}
{/if}
...
{elseif $t.views[$t.view].SHOW_PREVIEW && count($item.data)>1}
  ...
  <a target="_top" id="linktext" title="Preview" href="index.php?item[]={$data_item._id}&subitem={$key_filter}" onmousedown="event.cancelBubble=true;">
{/if}
