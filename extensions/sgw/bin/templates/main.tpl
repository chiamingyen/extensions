...
{if !$iframe}
  ...
  {if $t.att.ENABLE_CALENDAR}
	<link rel="alternate" type="application/atom+xml" title="Today Feed" href="index_php?folder={$t.folder}&view={$t.view}&export=rss&markdate=day&today=now 00:00&username=&password=">
	<link rel="alternate" type="application/atom+xml" title="Week Feed" href="index_php?folder={$t.folder}&view={$t.view}&export=rss&markdate=week&today=last monday&username=&password=">
	<link rel="alternate" type="application/atom+xml" title="Month Feed" href="index_php?folder={$t.folder}&view={$t.view}&export=rss&markdate=month&today={$smarty.now|modify::dateformat:"Y-m"}-01&username=&password=">
  {else}
	<link rel="alternate" type="application/atom+xml" title="Feed" href="index_php?folder={$t.folder}&view={$t.view}&export=rss&username=&password=">
  {/if}
{/if}
...
{if !$print}<noscript>Please enable Javascript in your browser.<br><br></noscript>{/if}
	  ...
	  {if ($t.datasets eq 0 && $t.schema_mode neq "new" && $t.schema_mode neq "static") || ($t.vright && !$t.rights[$t.vright])}
		<table cellspacing="0" class="data" style="margin-bottom:{if $iframe && !$t.data_day && !$t.data_month}0{else}2{/if}px;">
		  <tr><td style="padding-top:0px; padding-bottom:0px;">
		    {if $t.vright && !$t.rights[$t.vright]}Access denied.{else}No entries found.{/if}
		  </td></tr>
		</table>
	  {/if}
