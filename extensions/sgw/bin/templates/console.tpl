...
<body onload="start(); resizeit();" onresize="resizeit();">
  ...
  <div id="code" style="{if $content eq ""}width:100%;{/if}">
	...
	<div style="text-align:center;" id="buttons">
	  <input type="submit" value="    Execute  [ Alt+e ]    " accesskey="e">&nbsp;
	  {if $console eq "sql"}
		<input type="submit" name="full_texts" value=" Execute Full texts  [ Alt+f ] " accesskey="f">&nbsp;
		<input type="submit" name="vertical" value=" Execute vertical  [ Alt+v ] " accesskey="v">&nbsp;
	  {/if}
	  <input type="button" value=" Clear " onclick="obj('codebox').value=''; obj('codebox').focus();">
	  <input type="hidden" name="console" value="{$console|escape:"html"}">&nbsp;
	  {if $console neq "sql"}Time limit (seconds): <input type="text" name="tlimit" value="{$tlimit|escape:"html"}" style="width:40px;" />&nbsp;{/if}
	  Memory limit (MB): <input type="text" name="mlimit" value="{$mlimit|escape:"html"}" style="width:34px;" />
	</div>
	</form>
  </div>
