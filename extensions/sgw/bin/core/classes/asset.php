...
static function delete_items($folder,$view,$items,$mode="delete") {
  ...
  if ($delete) {
    ...
    if (empty($trash)) exit("Error: Trash folder not found.");
    ...
    foreach ($rows as $row) {
	  ...
	  $data = array("folder"=>$tid,"history"=>sprintf("Item deleted by %s at %s\n",$_SESSION["username"],sys_date("m/d/y g:i:s a")));
...
static function build_history($type, $value, $data_old) {
  ...
  switch($type) {
	...
	case "date": $value = sys_date("m/d/Y",$value); break;
   	case "datetime": $value = sys_date("m/d/Y g:i a",$value); break;
	case "time": $value = sys_date("g:i a",$value); break;
	case "dateselect":
	  $data = array();
	  foreach (explode("|",trim($value,"|")) as $date) $data[] = sys_date("m/d/Y", $date);
	  $value = implode(", ", $data);
	  break;
	case "files":
	...
	case "checkbox": $value = $value?"yes":"no"; break;
...
static function build_notification($module,$fields,$data_full,$data,$id,$data_row=array()) {
  ...
  if (!empty($data_full["notification_summary"])) {
    $message .= "Summary: ".trim($data_full["notification_summary"])."\n\n";
  }
  $title = "";
...
static function create_edit($tfolder, $tview, $mode) {
  ...
  foreach ($_REQUEST["form_fields"] as $id) {
	...
	if (!sys_validate_token()) $result['token'] = array(array("validation failed", "Invalid security token Please activate cookies."));
  ...
  if (isset($_FILES[$fieldname]) and is_array($_FILES[$fieldname])) {
	...
	foreach (array_keys($data["name"]) as $filenum) {
	  if ($data["error"][$filenum]=="0" and $data["size"][$filenum]!=0) {
		...
		$error[] = array($displayname,"Upload failed: ".$message);
  } } }
  
  if (!empty($field["SIMPLE_SIZE"]) and count($_REQUEST[$fieldname])>$field["SIMPLE_SIZE"]) {
	$error[] = array($displayname,"maximum number of files exceeded. (".$field["SIMPLE_SIZE"].")");
  }
  return $error;
}
...
private static function _PHPDiff($old,$new,$details) {
  ...
  foreach($actions as $act) {
    ...
    if ($op>0) {
      ...
      if ($op==1 and $details) $out[] = "deleted line ({$xstr})";
        elseif ($op==3 and $details) $out[] = "changed line ({$xstr})";
      while ($x0<$x1) { $out[] = '- '.trim($t1[$x0]); $x0++; } # deleted elems
      if ($op==2 and $details) $out[] = "added line ({$x1})";
