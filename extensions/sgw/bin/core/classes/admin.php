  ...
  switch ($_REQUEST["action_sys"]) {
    case "maintenance":
	  ...
	  if (!file_exists($lock_file)) {
	    touch($lock_file);
	    sys_log_message_alert("info","Maintenance mode: Active");
	  } else {
	    unlink($lock_file);
	    sys_log_message_alert("info","Maintenance mode: Inactive");
	  }
	  break;
    case "clear_locking":
	  self::_remove_locks();
	  dirs_create_empty_dir(SIMPLE_STORE."/locking");
	  sys_log_message_log("clean","Locking");
	  break;
    case "clear_output":
	  ...
	  sys_log_message_log("clean","Output");
	  break;
	case "clear_debug":
	  dirs_create_empty_dir(SIMPLE_CACHE."/debug");
	  sys_log_message_log("clean","Debug-dir");
	  break;
	case "clear_cms":
	  dirs_create_empty_dir(SIMPLE_CACHE."/cms");
	  sys_log_message_log("clean","CMS");
	  break;
	case "clear_ip":
	...
	case "clean_statistics":
	  ...
	  sys_log_message_log("clean","Statistics");
	  break;
	case "clean_events":
	  if (!sys_validate_token()) sys_die("Invalid security token");
	  ...
	  sys_log_message_log("clean","Events");
	  break;
	case "clean_trash":
	  if (!sys_validate_token()) sys_die("Invalid security token");
	  ...
	  sys_log_message_log("clean","Trash");
	...
	case "clean_cache": 
	  ...
	  sys_log_message_log("clean","Clean Cache");
	  sys_redirect("index.php?".sys::$urladdon);
	  break;
	case "edit_setup":
	...
	case "backup":
	  ...
	  self::_out("<br><a href='index.php'>Continue</a>");
	  exit;
	  break;
	case "restore_newer":
    ...
    case "clear_session":
	  ...
	  if (APC) apc_clear_cache("user");
	  sys_log_message_log("clean","Sessions");
	  self::_out("<br><a href='index.php'>Continue</a>");
	  exit;
	  break;
    case "phpinfo":
...
static function rebuild_schema($rebuild_search=false) {
  if ($rebuild_search) {
    self::_out(sprintf("Processing %s ...","schema, search index"));
  } else {
	self::_out(sprintf("Processing %s ...","schema"));
  }
...
private static function _create_backup($folder) {
  ...
  $message = "Backup: ".$folder_path." [".$bfolder["id"]."]";
...
private static function _restore($filename) {
  ...
  if (filesize($filename)==0) return "No entries found. (".$filename.")";
  
  self::_out("Extracting files: ".$filename);
  self::_out("");
  ...
  if (self::$_restore_here) {
	...
	self::_out("Insert: simple_sys_tree: ".$ftitle." [".$id."]");
  }
  ...
  foreach ($xml->table as $data) {
    ...
    if (empty($existing["id"]) or self::$_restore_here) {
	  ...
	  self::_out("Insert: simple_sys_tree: ".$data["ftitle"]." [ID ".$id." -> parent/id: ".$data["parent"]."/".$id2."]");
	  $restore_maps[$id] = $id2;
	  $id = $id2;
	} else {
	...
	if (!self::$_restore_missing and (!self::$_restore_onlynewer or $data["lastmodified"] > $existing["lastmodified"])) {
	  self::_out("Update: simple_sys_tree ".$id);
  ...
  foreach ($xml->table as $table_item) {
	...
	foreach ($table_item->asset as $asset) {
      ...
      if (empty($existing["id"]) or self::$_restore_here) {
	    ...
	    self::_out("Insert: ".$table.": ".$data["id"]);
  ...
  if (count($update_folders)>0) {
	foreach ($update_folders as $folder=>$table) {
	  ...
	  foreach ($update_ids[$folder] as $id) {
		self::_out("Rebuild search index: ".$table." [".$id."]");
		db_search_update($table,$id,$fields);
  } } }
  self::_out("");
  $message = "Restore complete: ".str_replace(array("__","---","--"),array("/","] ["," ["),substr(modify::basename($filename),0,-4))."]";
  sys_log_message_log("info",$message);
  self::_out($message);
  return "";
}
