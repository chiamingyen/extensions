...
static function require_group($value, $params) {
  ...
  if ((empty($values) or in_array($value, $values)) and !self::_has_a_group($groups)) {
	return sprintf("Value '%s' needs a group membership: %s", $value, implode(" or ", $groups));
  }
  return "";
}
...
static function json($value) {
  if (!json_decode($value)) return "Error: validation failed (JSON)";
  return "";
}
...
static function xml($value) {
  try {
    ...
    return "Error: ".$e->getMessage()." ".libxml_get_last_error()->message;
  }
  return "";
}
...
static function fileupload($filename,$params) {
  ...
  foreach ($filename as $file) {
    ...
    if (!in_array($ext, $exts)) return sprintf("Filename must have the right extension. Valid extensions are %s", $params[0]);
  }
  return "";
}
...
static function check_uploaded_file($files,$unused=null,$field=array()) {
  ...
  foreach ($files as $file) {
	...
	if ($size!=0 and filesize($file)>$size) {
	  return "Error: file is too big. Please upload a smaller one. (".modify::basename($file)." > ".$field["SIMPLE_FILE_SIZE"].")";
	}
    $ext = modify::getfileext($file);
    if (in_array($ext, $exts)) return sprintf("this file extension is not allowed (%s)", $ext);
  }
  return "";
}
...
static function checkvirus($files) {
  ...
  foreach ($files as $file) {
	...
	if ($bin=="") return sprintf("checkvirus: unable to find %s",VIRUS_SCANNER);
...
static function username($username) {
  if (defined("SETUP_ADMIN_USER") and sys_is_super_admin($username)) return "username: Username must be different from the super administrator.";
  if (strlen($username)<129 and strlen($username)>2 and preg_match('/^[a-z0-9-_@\.]*$/', $username)) return "";
  return "Name must be not null, lowercase, min 3 characters, max 128 containing [a-z0-9_-@.].";
}

static function regexp($value,$params) {
  if (empty($params[0])) return "";
  if (empty($params[1])) $params[1] = "validation failed: ".$params[0];
...
static function password($password) {
  ...
  return "Password must be not null, min 5 characters.";
}

static function email($email) {
  if (strlen($email)>5 and preg_match("/^[\S]+@[\S]+\.[\S]+$/si", $email)) return "";
  return "Please input a valid e-mail adress.";
}
...
static function numeric($var) {
  ...
  return "Variable must be of type int and greater than 0.";
}

static function integer($var) {
  if (is_numeric($var) and ceil($var)==$var) return "";
  return "Variable must be of type int.";
}

static function float($var) {
  if (is_numeric($var)) return "";
  return "Variable must be of type float.";
}

static function date($var) {
  if (strtotime(modify::date_translate($var))!=0) return "";
  return "Variable must be of type date (mm/dd/yyyy).";
}

static function time($var) {
  if (strtotime(modify::date_translate($var))!=0) return "";
  return "Variable must be of type time (hh:mm am/pm).";
}

static function datetime($var) {
  if (strtotime(modify::date_translate($var))!=0) return "";
  return "Variable must be of type datetime (mm/dd/yyyy hh:mm am/pm).";
}
...
static function itemexists($table,$vals,$id,$checktrash=false) {
  ...
  if (!$found) return "";
  return sprintf("Variable already exists in table. (%s)",implode(", ",$msgs));
}
