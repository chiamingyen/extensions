    ...
    if (empty($duplicate)) {
	  ...
	  */
	  ...
	  $data = array(
		...
		"fquota"=>0, "history"=>sprintf("Item created by %s at %s",$_SESSION["username"],sys_date("m/d/y g:i:s a"))
	  );
...
static function delete($folder) {
  ...
  if (!folder_in_trash($folder)) {
	...
	$id = self::create(sys_date("m/d/Y"),"blank","",$trash,true);
	$old_path = modify::getpath($folder);
	if (!self::move($row["id"],$id,true)) exit("The folder cannot be deleted.");
