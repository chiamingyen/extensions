...
/*
  ...
  <field name="image" displayname="Image" simple_type="files" simple_file_size="1M" simple_size="1">
  <field name="attachment" displayname="Attachment" simple_type="files" simple_file_size="2M" simple_size="1">
*/
    }
  }
...
private static function _vcard_data() {
  ...
  if (count($data)==0) {
	$output .= "BEGIN:VCARD\nVERSION:3.0\n";
	$output .= sys_remove_trans("FN:No entries found.\nN:No entries found.;---\nEND:VCARD\n");
  } else {
    $url = self::_url_folder();
    foreach ($data as $asset) {
      $output .= "BEGIN:VCARD\nVERSION:3.0\n";
...
";
  ...
  if (count($data)>0) {
    ...
    foreach ($data[0] as $field) {
	  ...
	  $output .= "&nbsp;<a title='Hide' class='hide_field' href='javascript:hide_column(".$i.");'>&ndash;</a></td>";
	  $i++;
    }
    $output .= "</tr>";
    foreach ($data as $asset) {
...
private static function _sss_editor($output) {
  echo '  
    ...
    <script src="ext/lib/simple_spreadsheet/translations/en.js" type="text/javascript"></script>
...
private static function _build_filename() {
  $t = $GLOBALS["t"];
  return "data-".$t["title"]."-".$t["folder"]."-".$t["view"]."-".$_SESSION["username"]."-".str_replace("/","_",sys_date("m/d/Y"));
}
...
private static function _build_data($html,$data_arr=array(),$xml=false,$rss=false,$hidden=false) {
  ...
  foreach ($data_arr as $asset) {
	...
	if (!in_array("id",$t["hidden_fields"]) and is_numeric($asset["_id"])) {
	  $data[$i]["id"] = array("name"=>"Id", "displayname"=>sys_remove_trans("Id"),"type"=>"","field"=>"id","filter"=>$asset["_id"],"width"=>"");
	}
	foreach ($t["fields"] as $akey=>$value) {
	  ...
	  if ($value["SIMPLE_TYPE"]=="checkbox") {
		if ($aval["data"][0]=="1") $aval["filter"][0] = "yes"; else $aval["filter"][0] = "";
	  }
	  if ($value["SIMPLE_TYPE"]=="date") $aval["filter"][0] = modify::dateformat($aval["data"][0],"m/d/Y");
	  if ($value["SIMPLE_TYPE"]=="datetime") $aval["filter"][0] = modify::dateformat($aval["data"][0],"m/d/Y g:i a");
	  $filter = "";
	...
	if (!empty($asset["lastmodifiedby"]) and !isset($data[$i]["lastmodifiedby"]) and $html) {
	  if (!in_array("lastmodified",$t["hidden_fields"])) {
	    $data[$i]["lastmodified"] = array("name"=>"lastmodified", "displayname"=>sys_remove_trans("Modified"),"type"=>"","field"=>"lastmodified","filter"=>sys_date("m/d/Y", $asset["lastmodified"]),"width"=>"");
	  }
	  if (!in_array("lastmodifiedby",$t["hidden_fields"])) {
	    $data[$i]["lastmodifiedby"] = array("name"=>"lastmodifiedby", "displayname"=>sys_remove_trans("Modified by"),"type"=>"","field"=>"lastmodifiedby","filter"=>$asset["lastmodifiedby"],"width"=>"");
	  }
	}
