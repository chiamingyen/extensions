...
static function exec($id, $data, $params) {
  ...
  if ($cmd=="") return "";
  sys_notification(sprintf("System command executed: %s", $cmd));
  return sys_exec($cmd);
}
...
static function createuser($id, $data) {
  ...
  if (!is_dir($folder)) {
	sys_mkdir($folder);
	sys_notification(sprintf("Folder created. (%s)", $folder));
  }
...
static function deleteuser($id, $data) {
  ...
  if (!empty($count) and !empty($row_id)) {
	...
	sys_notification(sprintf("Folder structure moved to trash. (%s)", $data["username"]));
  }
...
static function deletegroup_by_name($name) {
  ...
  sys_notification(sprintf("Group deactivated. (%s)", $name));
  return "";
}
...
static function creategroup($name) {
  ...
  if (empty($row_id) and !empty($folder)) {
	...
	if ($error_sql=="") {
	  ...
	  sys_notification(sprintf("Group created. (%s)", $name));
...
static function editgroup($id, $data) {
  ...
  if (sys_strbegins($data["groupname"],"project_")) {
	...
	sys_notification("Update Participants: Project ".$project);
  }
  if (sys_strbegins($data["groupname"],"department_")) {
	...
	sys_notification("Update Members: Department ".$department);
  }
  return "";
}
...
static function deleteproject($id, $data) {
  ...
  if (!empty($count) and !empty($row_id)) {
	folders::delete($row_id);
	sys_notification(sprintf("Folder structure moved to trash. (%s)", $data["projectname"]));
  }
...
static function deletedepartment($id, $data) {
  ...
  if (!empty($count) and !empty($row_id)) {
	folders::delete($row_id);
	sys_notification(sprintf("Folder structure moved to trash. (%s)", $data["departmentname"]));
  }
...
static function sendmail($id, $data, $unused, $table, $to_self=false) {
  ...
  if (USE_MAIL_FUNCTION or !empty($row["smtp"])) {
	...
	if (empty($result) and sys_strbegins($data["subject"],SMTP_NOTIFICATION)) {
	  sys_notification(sprintf("Notificaiton sent to: %s", $data["eto"]));
    }
  } else {
    $result = sprintf("Mail identities: SMTP not configured for %s",$_SESSION["username"]);
  }
  return $result;
}
...
static function runxml($id, $data, $params) {
  ...
  if (!empty($home) and file_exists(sys_custom($file))) {
	sys_notification(sprintf("Processing %s ...", $file));
    $folder = folders::create_default_folders($file,$home,false,$data);
	sys_notification(sprintf("Folder structure created. (%s)", modify::getpath($folder)." / "));
  }
  return "";
}
...
static function createedit_payroll($id, $data, $unused, $table) {
  ...
  if ($data["status"]!="open") {
	...
	if (count($cdata)>0) {
	  ...
	  sys_notification("Asset marked as read-only.");
	}
	if (self::_add_asset_permission_group($table,$id,array_merge($data,$cdata),"admin_payroll")) {
	  sys_notification(sprintf("Permissions added for: %s", "admin_payroll"));
	}
  }
  return "";
}
