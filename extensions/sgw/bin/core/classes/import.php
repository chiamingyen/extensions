...
class import extends Spreadsheet_Excel_Reader {
  ...
  function process_row($row, $line) {
	...
	if (empty($result)) { // validate
	  ...
	  $message = sprintf("line %s: %s", $line, self::err_to_str($result));
	  $this->_errors[] = $message;
	  $this->out("<span style='color:red; font-weight:bold;'>Error:</span> ".modify::htmlquote($message).", ");
	} else {
  ...
  static function err_to_str($error) {
	...
	foreach ($error as $field) {
	  foreach ($field as $error) $result[] = "Column \"".$error[0]."\": ".$error[1];
	}
	return implode(", ", $result);
  }
  
  static function get_fields($sgsml) {
	$fields = array("id"=>sys_remove_trans("Id"), "folder"=>sys_remove_trans("Folder"));
	$view = $sgsml->view;
