...
static function mountpoints($admin=false) {
  $results = array(
	"sys_nodb_bookmarks" => "Bookmarks",
	"sys_nodb_csv_contacts" => "Contacts (CSV)",
	"sys_nodb_ldif_contacts" => "Contacts (LDIF)",
	"sys_nodb_icalendar" => "iCalendar",
	"sys_nodb_csv_calendar" => "Calendar (CSV)",
...
static function timezones($default = false) {
  ...
  if ($default) $timezones = array_merge(array("" => "Default"), $timezones);
  return $timezones;
}
