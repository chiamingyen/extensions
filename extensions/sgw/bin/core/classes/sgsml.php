...
function __construct($folder,$view,$items=array(),$writeable=true) {
  ...
  if (!is_numeric($folder)) {
    ...
    if (empty($row["ftype"])) throw new Exception("Folder not found.");
	$type = $row["ftype"];
	if ($row["folders"]!="") {
	  $folders = array();
  ...
  if (isset($_SESSION["disabled_modules"][$type])) exit("Module disabled.");
...
function validate_field($field_name, $value, $id=-1) {
  ...
  if (isset($field["REQUIRED"]) and $value_str=="") $error[] = array($key_name,"missing field");
...
private function _complete_data($data, $id=-1) {
  ...
  if (!$insert) {
	...
	if (!$data_row) return array(false,false,self::_error("Data","Item(s) not found or access denied."));
  }
  $rdata = array();
	...
	if ($field["SIMPLE_TYPE"]=="files" and !empty($rdata[$field_name])) { // download from URL
	  foreach ($rdata[$field_name] as $filekey=>$file) {
		...
		if ($target=="" or !file_exists($target)) {
		  return array(false,false,self::_error("Data","Upload failed: The url doesn't exist. ".$file));
		}
...
private function _save(array &$data, $id=-1) {
  ...
  if (!empty($this->att["DEFAULT_SQL"]) and $this->att["DEFAULT_SQL"]=="no_select") {
    return self::_error("Module","Access denied.");
  }

  if (!empty($data["folder"])) {
	// check permissions
	if (!db_get_right($data["folder"], "write", $this->view)) return self::_error("Folder","Access denied.","folder");
  ...
  if ($insert) {
	...
	$sql_data = array("id"=>$id,"dsize"=>0,"history"=>sprintf("Item created by %s at %s\n",$_SESSION["username"],sys_date("m/d/y g:i:s a")));
  } else {
    $sql_data = array("dsize"=>0,"history"=>sprintf("Item edited (%s) by %s at %s\n","@fields@",$_SESSION["username"],sys_date("m/d/y g:i:s a")));
  }
  ...
  if ($insert) {
	...
	if ($error_sql!="") return self::_error("SQL failed.",$error_sql);
	if ($this->notification) sys_notification("Item successfully created. (".$id.")");
  } else {
	...
	if ($error_sql!="") return self::_error("SQL failed.",$error_sql);
	if ($this->notification) sys_notification("Item successfully updated. (".(is_numeric($id)?$id:1).")");
  }
  ...
  if ($trigger and ($result = asset_process_trigger($trigger,$id,$rdata,$this->tname))) {
	return self::_error("Trigger failed",$result);
  }
  ...
  if (!empty($rdata["notification"])) {
	...
	$smtp_data = asset::build_notification($this->att["NAME"],$this->current_fields,$rdata,$sql_data,$id,$data_row);
  	if (($result = asset_process_trigger("sendmail",$id,$smtp_data))) return self::_error("Trigger failed",$result);
  }
