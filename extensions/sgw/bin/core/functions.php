...
class sys {
  ...
  static function shutdown() {
	...
	if (self::$time_end > SYSTEM_SLOW) {
	  sys_log_message_log("system-slow",sprintf("%s secs",self::$time_end)." ".basename(_sys_request_uri()),_sys_request_uri());
	}
...
function _asset_get_rows() {
  ...
  if ($mode=="edit" and is_array($rows) and !empty($t["sqlvars"]["item"]) and count($rows) < count($t["sqlvars"]["item"])) {
	sys_warning("Item(s) not found or access denied.");
  }
	  ...
	  if (file_exists($lckfile)) {
	    ...
	    if (($lcktime+LOCKING) > time()) {
	      ...
	      if ($lckuser!=$_SESSION["username"]) {
	        $t["data"][$row_key]["_lck"] = sprintf("%s started editing this asset at %s",$lckuser,sys_date("g:i a",$lcktime));
	  } } }
	  if ($lckuser!=$_SESSION["username"]) {
	...
	foreach (explode("||",$t["filter"]) as $key=>$filter) {
	  ...
	  if (count($filter)==3 and isset($t["fields"][$filter[0]]) and in_array($filter[1],$ops)) {
		...
		if ($t["fields"][$filter[0]]["SIMPLE_TYPE"]=="checkbox") {
		  $filter[2] = $filter[2]==sys_remove_trans("yes")?"1":"0";
		}
...
function db_check_quota($tfolder) {
  ...
  if ($quota["remain"] < 0) {
	sys_log_message_log("quota",sprintf("Quota limitation exceeded: %s",modify::filesize($quota["remain"])." id: ".$tfolder));
	sys_warning(sprintf("Quota limitation exceeded: %s",modify::filesize($quota["remain"])));
  }
}
  ...
  foreach ($row as $data_key=>$data) {
    ...
    if (isset($fields[$data_key]) and $data!="" and (!is_numeric($data) or $data!=0) and $data!="null") {
	  ...
	  switch ($field["SIMPLE_TYPE"]) {
		...
		case "time": $data = sys_date("g:i a",$data); $data2 = $data; break;
	    case "date": $data = sys_date("m/d/Y",$data); $data2 = $data; break;
		case "dateselect": $data2 = ""; foreach (explode("|",$data) as $date) $data2 .= " ".sys_date("m/d/Y g:i a",$date); $data = $data2; break;
	    case "datetime": $data = sys_date("m/d/Y g:i a",$data); $data2 = $data; break;
		case "files":
		  $data2 = "";
...
function db_get_schema($schema_file, $folder="", $tview="", $cache=true, $popup=false) {
  ...
  if (!file_exists($schema_file)) {
    if (basename($schema_file)=="nodb_.xml") {
	  sys_warning(sprintf("Folder not found. (%s)", $folder));
	} else {
	  sys_log_message_alert("php-fail", sprintf("Schemafile not found. (%s)", $schema_file." ".$folder));
	}
  ...
  if (empty($data[$cid])) {
    ...
    if (defined("SETUP_DB_HOST")) {
	  sys_log_message_log("info",sprintf("Updating schema %s from %s. Folder: %s",$schema,$schema_file,$folder));
	}
  }
  if ($tview=="") return $data[$cid];
  ...
  if (isset($data[$cid][$tview])) {
	...
	if ($tview!="none") sys_warning("Item(s) not found or access denied. (view={$tview})");
...
function db_query($sql, $vars=array()) {
  ...
  if ($time > DB_SLOW) {
	...
	sys_log_message_log("db-slow",sprintf("%s secs",$time)." ".$sql,sys_backtrace()." ".$slow_log);
  }
...
function debug_handler($errno, $errstr, $errfile, $errline) {
  ...
  $err = "Php ".(isset($errortype[$errno])?$errortype[$errno]:$errno).": ".sys_date("m/d/y g:i:s a")." ".$errstr." ".$errfile." ".$errline;
...
function debug_sql($var,$errmsg) {
  ...
  $trace = "# ".sys_backtrace()." ".sys_date("m/d/y g:i:s a")."\n".preg_replace("/(\s+)/"," ",str_replace(array("\n","\r"),"",$var)).";".($errmsg!=""?"\n".$errmsg:"")."\r\n\r\n";
...
function dirs_create_index_htm($path) {
  foreach (array("index.htm","index.html") as $file) {
    if (!file_exists($path.$file)) {
	  if (!@file_put_contents($path.$file, "You are not allowed to view this folder.", LOCK_EX)) {
	    sys_die(sprintf("Unable to write to %s",$path.$file));
} } } }
...
function dirs_checkdir($path) {
  if (!is_dir($path) and !sys_mkdir($path)) sys_die(sprintf("Failed to create directory: %s",$path));
  if (!is_writable($path)) sys_die(sprintf("Failed to write into %s",$path));
  dirs_create_index_htm($path);
}
...
function folder_process_session_request() {
  ...
  if (isset($_REQUEST["fschema"])) {
	...
	if (!isset($row["id"])) {
	  sys_warning("Item not found. (".$_REQUEST["fschema"].")");
...
function folder_build_selfolder($tfolder,$tview) {
  if (!is_numeric($tfolder)) {
    ...
    if (db_get_right($tfolder,"read")) {
	  ...
	  if (!empty($_REQUEST["iframe"]) and $tfolder!=1) sys_die("Access denied.");
    }
	if (!is_array($sel_folder)) sys_die("ERROR ".sql_error());
	if (count($sel_folder)==0) sys_die("Access denied. <a href='index.php?logout'>Login/-out</a>");
...
function login_handle_login($save_session=true) {
  ...
  if (!empty($_REQUEST["username"]) and !empty($_REQUEST["password"]) and (!$maintenance or sys_is_super_admin($_REQUEST["username"]))) {
    if (!isset($_COOKIE[SESSION_NAME]) and !empty($_REQUEST["loginform"])) sys_die('Please activate cookies. <a href="index.php?logout">Back</a>');
  ...
  if ($maintenance and (empty($_SESSION["username"]) or !sys_is_super_admin($_SESSION["username"]))) {
	$_REQUEST["logout"] = true;
	sys_alert("Maintenance mode: Active.");
  }
...
function sys_allowedpath($dir) {
  ...
  if (!is_dir($dir) or $dir=="/") return "directory does not exist.";
  ...
  if ($found) {
    ...
    sys_warning(sprintf("Directory is outside the allowed directory. (%s is outside %s)",$path_filename,implode(", ",$_SESSION["ALLOWED_PATH"])));
...
function sys_get_xml($file, $data = array()) {
  ...
  try {
    ...
    sys_die("Error: ".$file." ".$e->getMessage()." ".libxml_get_last_error()->message);
  }
  return new stdClass();
}
...
function sys_find_callback($class,$function) {
  ...
  if (!method_exists($class, $function)) {
    $message = sprintf("Function does not exist: %s",$class."::".$function);
...
function sys_process_output() {
  if (empty($_REQUEST["export"]) and empty($_REQUEST["import"])) {
	...
	if (method_exists("export",$_REQUEST["export"])) call_user_func(array("export", $_REQUEST["export"]));
	  else sys_die(sprintf("Function does not exist: %s","export::".$_REQUEST["export"]));
  }
}
...
function sys_message_box($subject, $ret) {
  ...
  $out .= "<div><a style='float:right;' onclick='javascript:this.parentNode.parentNode.style.display=\"none\";'>Close</a></div>";
