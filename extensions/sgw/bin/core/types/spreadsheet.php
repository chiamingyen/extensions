...
static function form_render_value($name, $value, $smarty) {
  ...
  return <<<EOT
	...
	<div style="margin-bottom:2px; width:100%; height:{$item_size}px;">
	  <iframe id="{$name}_iframe" style="margin:0px; padding:0px; width:100%; height:100%;" src="ext/lib/simple_spreadsheet/spreadsheet.php?mode=editor&lang=en&data={$name}"></iframe>
	</div>
EOT;
}
  ...
  if ($preview) {
    return <<<EOT
	  ...
	  <div style="margin:2px; width:100%; height:{$height}px;">
		<iframe style="margin:0px; padding:0px; width:99%; height:100%;" src="ext/lib/simple_spreadsheet/spreadsheet.php?mode=viewer&lang=en&data={$id}"></iframe>
	  </div>
EOT;
  }
