...
if (!in_array(".",$include_path)) {
  setup_die(sprintf("Please modify your php.ini or add an .htaccess file changing the setting '%s' to '%s' (current value is '%s') !","include_path",".".$sep.implode($sep,$include_path),ini_get("include_path")),1);
}

$phpversion = "5.2.0";
if (version_compare(PHP_VERSION, $phpversion, "<")) {
  setup_die(sprintf("Setup needs php with at least version %s ! (".PHP_VERSION.")",$phpversion),3);
}
...
if (!empty($_SERVER["SERVER_SOFTWARE"]) and !preg_match("/Apache|nginx|IIS/", $_SERVER["SERVER_SOFTWARE"])) {
  setup::error("Please choose Apache as Web-Server. (".$_SERVER["SERVER_SOFTWARE"].")","2".$_SERVER["SERVER_SOFTWARE"]);
}

$memory = ini_get("memory_limit");
if (!empty($memory)) {
  $memory = (int)str_replace("m","000000",strtolower($memory));
  if ($memory < $memorylimit) setup::error(sprintf("Please modify your php.ini or add an .htaccess file changing the setting '%s' to '%s' (current value is '%s') !","memory_limit",str_replace("000000","M",$memorylimit),ini_get("memory_limit")),4);
}
...
foreach($extensions as $key => $key2) if (!in_array($key2, $sys_extensions)) setup::error(sprintf("Setup needs php-extension with name %s !",$key2),"5".$key2);
...
if (count($GLOBALS["databases"])==0) setup::error(sprintf("Setup needs a database-php-extension ! (%s)",implode(", ",$db_extensions)),6);

foreach ($settings as $setting => $values) {
  if (!in_array(ini_get($setting), $values)) setup::error(sprintf("Please modify your php.ini or add an .htaccess file changing the setting '%s' to '%s' (current value is '%s') !",$setting,$values[0],ini_get($setting)),"7".$setting);
}
...
if (!is_writable(SIMPLE_CACHE."/") or !is_writable(SIMPLE_STORE."/")) {
  $message = sprintf("[1] Please give write access to %s and %s",SIMPLE_CACHE."/",SIMPLE_STORE."/");
  $message .= sprintf("\nIf file system permissions are ok, please check the configurations of %s if present.", "SELinux, suPHP, Suhosin");
  setup::error($message,8);
}
if (!is_writable("../bin/")) setup::error(sprintf("[2] Please give write access to %s","../bin/"),9);
if (!DEBUG and !is_writable("../bin/index.php")) setup::error(sprintf("[3] Please give write access to %s","../bin/index.php"),10);
if (!is_readable("../lang/")) setup::error(sprintf("[2] Please give read access to %s","../lang/"),11);
if (!is_readable("../import/")) setup::error(sprintf("[2] Please give read access to %s","../import/"),111);
...
function setup_die($str,$err) {
  echo '
	...
	<br><br>
	<a href="index.php">Relaunch Setup</a>
	<br><br><hr>
	<a href="http://www.simple-groupware.de/cms/Main/Installation" target="_blank">Installation manual</a> / 
	<a href="http://www.simple-groupware.de/cms/Main/Update" target="_blank">Update manual</a><hr>
	<a href="http://www.simple-groupware.de/cms/Main/Documentation" target="_blank">Documentation</a> / 
...
function show_form() {
  ...
  echo '
	...
	'.($globals?sprintf("Warning: Please modify your php.ini or add an .htaccess file changing the setting '%s' to '%s' (current value is '%s') !<br><br>","register_globals","0",$globals):"").'
	'.($mb_string?sprintf("Warning: Please install the php-extension with name '%s'.<br><br>","mbstring"):"").'
	'.((isset($_REQUEST["install"]) and empty($_REQUEST["accept_gpl"]))?"&nbsp;=&gt; To continue installing Simple Groupware you must check the box under the license<br><br>":"").'
	</b></div>
	<form action="index.php" method="post">
	<input type="hidden" value="en" name="language">
	<table class="data">
	<tr id="db_host_row">
	  <td><label for="db_host">Database Hostname / IP</label></td>
	...
	<tr id="db_pw_row">
	  <td><label for="db_pw">Database Password</label></td>
	...
	<tr>
	  <td><label for="db_type">Database</label></td>
	  <td>
  ';
  ...
  echo '
	...
	<tr>
	  <td><label for="admin_pw">Admin Password</label></td>
	  <td><input type="text" value="" size="30" maxlength="50" name="admin_pw" id="admin_pw" required="true"></td>
	</tr>
	<tr>
	  <td><label for="folders">Folder structure</label></td>
	  <td>
		<select name="folders" id="folders">
		  <option value="modules/core/folders.xml">Install demo folders
		  <option value="modules/core/folders_small.xml">Install default folder structure
		  <option value="modules/core/folders_minimal.xml">Install minimal folder structure
		</select>
	  </td>
	</tr>
	</table>
    <div style="border-bottom: 1px solid black;">&nbsp;</div>
	<h2>GNU GPL License Version 2</h2>
	<h4>
	<a href="http://www.gnu.org/copyleft/gpl.html" target="_blank">More information about the GNU GPL</a><br>
	<a href="http://www.gnu.org/licenses/translations.html" target="_blank">Translations of the GNU GPL</a><br> 
	<a href="http://www.gnu.org/licenses/gpl-faq.html" target="_blank">GNU GPL Frequently Asked Questions</a>
	<br>
	</h4>
	<font color="#ff0000">*** To continue installing Simple Groupware you must check the box under the license ***</font><br><br>
	Please read the following license agreement. Use the scroll bar to view the rest of this agreement.<br>
    <div style="border-bottom: 1px solid black;">&nbsp;</div>
	<pre>'.trim(file_get_contents("LICENSE.txt")).'</pre>
    <div style="border-bottom: 1px solid black;">&nbsp;</div>
	<br>
	<div style="border: 2px solid #FF0000; width:400px;">&nbsp; <input onclick="if (this.checked) this.parentNode.style.border=\'2px solid #00A000\'; else this.parentNode.style.border=\'2px solid #FF0000\';" type="Checkbox" class="checkbox" name="accept_gpl" id="accept_gpl" value="yes" style="margin: 0px;" accesskey="a" required="true"> <label for="accept_gpl">I Accept the GNU GENERAL PUBLIC LICENSE VERSION 2</label></div>
	<br><br>
	<input type="submit" name="install" value="I n s t a l l" class="submit" style="width:400px;"><br><br>
	</form>
...
function install() {
  ...
  if ($validate=validate::username($_REQUEST["admin_user"]) and $validate!="") setup::error("Admin Username - validation failed ".$validate,30);
  if ($_REQUEST["db_host"]=="") setup::error(sprintf("missing field: %s","Database Hostname / IP"),31);
  if ($_REQUEST["db_user"]=="") setup::error(sprintf("missing field: %s","Database User"),32);
  if ($_REQUEST["db_name"]=="") setup::error(sprintf("missing field: %s","Database Name"),33);
  if ($_REQUEST["admin_pw"]=="") setup::error(sprintf("missing field: %s","Admin Password"),34);
  if ($_REQUEST["admin_pw"]!="" and strlen($_REQUEST["admin_pw"])<5) setup::error("Admin Password: Password must be not null, min 5 characters.","34b");

  define("SETUP_DB_TYPE",$_REQUEST["db_type"]);

  if (!@sql_connect($_REQUEST["db_host"], $_REQUEST["db_user"], $_REQUEST["db_pw"], $_REQUEST["db_name"])) {
    if (!sql_connect($_REQUEST["db_host"], $_REQUEST["db_user"], $_REQUEST["db_pw"])) setup::error("Connection to database failed.\n".sql_error(),35);
    if (empty(sys::$db)) setup::display_errors(true);
	if (!sgsml_parser::create_database($_REQUEST["db_name"])) setup::error("Creating database failed.\n".sql_error(),36);
  }
  if (!sql_connect($_REQUEST["db_host"], $_REQUEST["db_user"], $_REQUEST["db_pw"], $_REQUEST["db_name"]) or empty(sys::$db)) {
    setup::error("Connection to database failed.\n".sql_error(),37);
	setup::display_errors(true);
  }

  if (!$version = sgsml_parser::sql_version()) setup::error(sprintf("Could not determine database-version."),38);
  $database_version_min_int = (int)substr(str_replace(".","",$GLOBALS["db_min_version"][SETUP_DB_TYPE]),0,3);
  if ($version < $database_version_min_int) setup::error(sprintf("Wrong database-version (%s). Please use at least %s !",$version,$GLOBALS["db_min_version"][SETUP_DB_TYPE]),"20".SETUP_DB_TYPE);
  ...
  if (SETUP_DB_TYPE=="pgsql") {
	  ...
	  setup::error("Please install 'tsearch2' for the PostgreSQL database.\n(Run <postgresql>/share/contrib/tsearch2.sql)\n".sql_error(),21);
	}
    if (!sql_query(file_get_contents("modules/core/pgsql.sql"))) setup::error("pgsql.sql: ".sql_error(),50);
  }
  setup::out(sprintf("Processing %s ...","schema updates"));

  // 0.730
  $status = array("completed"=>"completed","confirmed"=>"confirmed","booked"=>"booked", "canceled"=>"canceled");
  ...
  if (sgsml_parser::table_column_exists("simple_timesheets","completed")) {
	db_update("simple_timesheets",array("status"=>"open"),array("completed=0", "status=@status@"),array("status"=>"unconfirmed"),array("no_defaults"=>1));
	db_update("simple_timesheets",array("status"=>"completed"),array("completed=1", "status=@status@"),array("status"=>"unconfirmed"),array("no_defaults"=>1));
  }
  if (sgsml_parser::table_column_exists("simple_expenses","completed")) {
	db_update("simple_expenses",array("status"=>"open"),array("completed=0", "status=@status@"),array("status"=>"unconfirmed"),array("no_defaults"=>1));
	db_update("simple_expenses",array("status"=>"completed"),array("completed=1", "status=@status@"),array("status"=>"unconfirmed"),array("no_defaults"=>1));
  }
  
  // 0.662
  $priority = array("lowest"=>"1", "low"=>"2", "normal"=>"3", "urgent"=>"4", "immediate"=>"5");
  ...
  if (sgsml_parser::table_column_exists("simple_sys_custom_fields","id")) {
	setup::out(sprintf("Processing %s ...","customization fields"));
  ...
  if (setup::get_config_old("SYNC4J",false,0) == "1") {
    setup::out(sprintf("Processing %s ...","Funambol schema"));
  ...
  db_update("simple_sys_tree",array("anchor"=>"locations"),array("ftype='locations'","flevel=2","ftitle='Rooms'"),array());

  // change anchor for demo, debug folder, workspace, organisation, 0.292
  db_update("simple_sys_tree",array("anchor"=>"demo"),array("ftype='blank'","flevel=1","ftitle='Demo'"),array());
  db_update("simple_sys_tree",array("anchor"=>"debug"),array("ftype='blank'","flevel=2","ftitle='Debug'"),array());
  db_update("simple_sys_tree",array("anchor"=>"workspace"),array("ftype='blank'","flevel=0"),array());
  db_update("simple_sys_tree",array("anchor"=>"organisation"),array("ftype='blank'","flevel=1","ftitle='Organisation'"),array());
  db_update("simple_sys_tree",array("anchor"=>"system"),array("ftype='sys_nodb_admin'","flevel=1","ftitle='System'"),array());
  ...
  if (!file_exists(SIMPLE_STORE."/setup_emails")) {
	setup::out(sprintf("Processing %s ...","emails message"));
  ...
  setup::out(sprintf("Processing %s ...","sessions"));
  ...
  if (!file_exists(SIMPLE_STORE."/setup_notify")) {
	...
	foreach ($notifications as $table=>$where) {
	  setup::out(sprintf("Processing %s ...",$table));
  ...
  setup::out(sprintf("Processing %s ...","default groups"));
  ...
  if (!empty($parent)) {
    ...
    if (empty($row_id)) {
	  folders::create("Backups","sys_nodb_backups","",$parent,false);
    }
    $row_id = folder_from_path("^trash");
    if (empty($row_id)) {
	  folders::create("Trash","blank","",$parent,false,array("anchor"=>"trash"));
    }
    $row_id = folder_from_path("~sys_notifications");
    if (empty($row_id)) {
	  folders::create("Notifications","sys_notifications","Delivery: cron.php",$parent,false);
    }
    $row_id = folder_from_path("^customize");
    if (empty($row_id)) {
	  folders::create("Customize","blank","",$parent,false,array("anchor"=>"customize"));
    }
    $row_id = folder_from_path("~sys_console");
    if (empty($row_id)) {
	  $id = folders::create("Console","sys_console","",$parent,false,array());
  ...
  if (empty($row_id) and !empty($parent)) {
	folders::create("Fields","sys_custom_fields","Customization rules\nfor modules based on sgsML",$parent,false);
  }
  $parent = folder_from_path("^workspace");
  $row_id = folder_from_path("^extensions");
  if (!empty($parent) and empty($row_id)) {
	folders::create("Extensions","blank","",$parent,false,array("anchor"=>"extensions"));
  }
  ...
  $vars = array(
	...
	"SMTP_REMINDER"=>"'Simple Groupware Reminder'",
	"SMTP_NOTIFICATION"=>"'Simple Groupware Notification'",
  ...
  if (file_put_contents(SIMPLE_STORE."/config.php", implode("\n",$out), LOCK_EX)) {
	...
	setup::out('<br><a href="index.php">C O N T I N U E</a><br><finished>');
	if (function_exists("memory_get_usage") and function_exists("memory_get_peak_usage")) {
	  setup::out("<!-- ".modify::filesize(memory_get_usage())." - ".modify::filesize(memory_get_peak_usage())." -->",false);
	}
