...
class cms {
  ...
  function __destruct() {
	...
	echo "<!-- ".$time."s -->";
	if ($time > CMS_SLOW) sys_log_message_log("cms-slow",sprintf("%s secs",$time)." ".$_REQUEST["page"],var_export($_REQUEST,true));
  ...
  function render_page() {
	...
	if (empty($this->page["id"])) {
	  if (PageDbStore::exists($pagename)) {
		...
		if (empty($this->page["id"])) sys_die("Page not found: ".$pagename.", Site.Authform");
	  } else {
	    header('HTTP/1.1 404 Not Found');
	    $this->page = PageDbStore::read("Site.PageNotFound");
	    if (empty($this->page["id"])) sys_die("Page not found: ".$pagename.", Site.PageNotFound");
  } } }
  ...
  function output() {
	...
	if ($output=="") {
	  sys_log_message_log("cms-fail",sprintf("Output empty: %s",$this->page["pagename"]." ".$this->page["template"]),var_export($_REQUEST,true));
  ...
  function render($pagename) {
    ...
    if (empty($page["id"])) {
	  ...
	  if (empty($page["id"])) sys_die("Page not found: ".$pagename.", Site.PageNotFound");
	}
  ...
  static function checkdos() {
	...
	if ($delay)	exit("<html><body><script>setTimeout('document.location.reload()',1500);</script>Please wait ...<noscript>Please hit reload.</noscript></body></html>");
  }
}