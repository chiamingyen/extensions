...
if ((empty($_REQUEST["release"]) and empty($_REQUEST["cfile"])) or !sys_validate_token()) {
  ...
  setup::out("Downloading update list ...<br>");
    ...
    foreach ($match[1] as $key=>$item) {
	  ...
	  if (!empty($match[2][$key])) {
		...
		if (!empty($match_version[1]) and version_compare(PHP_VERSION, $match_version[1], "<")) {
	      setup::out(sprintf("Setup needs php with at least version %s !", $match_version[1]));
		  $check = false;
		}
		...
		if (!empty($match_version[1])) {
		  ...
		  if ($curr_version < $db_version) {
		    setup::out(sprintf("Wrong database-version (%s). Please use at least %s !", $curr_version, $match_version[1]));
			$check = false;
	  } } }
	  if ($check) {
	    setup::out("<a href='updater.php?mirror=".$mirror_id."&token=".modify::get_form_token()."&release=".$item."'>I n s t a l l</a>&nbsp; Simple Groupware ", false);
	    setup::out($item." (<a target='_blank' href='http://www.simple-groupware.de/cms/Release-".str_replace(".","-",$item)."'>Changelog</a>)<br>");
	  }
	}
	if (!$found) setup::out("Already running latest release.<br>");
  } else {
    setup::out(sprintf("Connection error: %s [%s]", $mirror["url"], "HTTP")."<br>".strip_tags($data,"<br><p><h1><center>"));
  }

  setup::out("Server: <b>".$mirror["name"]."</b>, use mirror from: ", false);
  foreach ($mirrors as $key => $sel_mirror) {
	if ($key==$mirror_id) continue;
	setup::out("<a href='updater.php?mirror=".$key."'>".$sel_mirror["name"]."</a> ");
  }

  setup::out("<br/>Package from local file system (.tar.gz):<br/>current path: ".str_replace("\\","/",getcwd())."/<br/>");

  $dir = opendir("./");
  while (($file=readdir($dir))) {
    if ($file!="." and $file!=".." and preg_match("|^SimpleGroupware\_.*?.tar\.gz\$|i",$file)) {
	  setup::out("<a href='updater.php?token=".modify::get_form_token()."&cfile=".$file."'>I n s t a l l</a>&nbsp; ".$file."<br/>");
	}
  }
  closedir($dir);

  setup::out("<form method='POST'><input type='hidden' name='token' value='".modify::get_form_token()."'><input type='text' name='cfile' value='/tmp/SimpleGroupware_0.xyz.tar.gz' style='width:300px;'>&nbsp;<input type='submit' class='submit' value='I n s t a l l'><br>");

  setup::out("<input type='checkbox' name='nobackup' value='1' onchange='change_links(this.checked);'/> Don't move old files to 'old/'</form>");
  ...
  if (!file_exists($source) or filesize($source) < 3*1024*1024) sys_die("Error: file-check [0] ".$source);
} else {
  $release = "latest";
...
if ($fz = gzopen($source,"r") and $fp = fopen($target,"w")) {
  ...
  gzclose($fz);
  fclose($fp);
} else sys_die("Error: gzopen [1] ".$source);

setup::out();
if (!file_exists($target) or filesize($target) < 5*1024*1024) sys_die("Error: file-check [2] Filesize: ".filesize($target)." ".$target);
...
setup::out("<br><a href='index.php'>C O N T I N U E</a><finished>");
