...
class Net_SMTP
{
     ...
     * @access public
     */
    var $auth_methods = array('DIGEST-MD5', 'LOGIN', 'PLAIN', 'CRAM-MD5');
    ...
    function _parseResponse($valid, $later = false)
    {
        ...
        return PEAR::raiseError('Invalid response code received from server: '.$output,
                                $this->_code);
    }
    ...
    function mailFrom($sender, $params = null)
    {
        ...
        if (PEAR::isError($error = $this->_put('MAIL', $args))) {
		    $error->message .= " ".implode(", ",$this->_arguments);
            return $error;
        }
