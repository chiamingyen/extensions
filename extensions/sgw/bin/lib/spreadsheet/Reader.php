                ...
                case SPREADSHEET_EXCEL_READER_TYPE_XF:
                        ...
                        //echo "isnumber ".$this->numberFormats[$indexCode];
                            ...
                            if ($indexCode > 0){
								...
								// echo "##\n".$formatstr."##<br>\n";
                                if ($formatstr && preg_match("/[DMYHM:]/i", $formatstr)) { // found day and time format
                                    $isdate = TRUE;
									$replace = array(
										"AM/PM" => "",
										"YYYY" => "YY",
                                    ...
                                    $formatstr = trim(str_replace(array_keys($replace), array_values($replace), $formatstr));
                                    if (DEBUG) echo "\ndate-time $formatstr #_<br>";
                                }
                            }
...
/* Unicode sheet names?
+                            $compressedUTF16 = ((ord($this->data[$pos+11]) & 0x01) == 0) ;
+                            $rec_length = ($compressedUTF16)? $rec_length : $rec_length*2;
+                            $rec_name    = $this->_encodeUTF16(substr($this->data, $pos+12, $rec_length), $compressedUTF16);
*/
...
//            $r = &$this->file->nextRecord();
    ...
    function createDate($numValue)
    {
        if ($numValue > 1) {
            ...
            $string = gmdate ($this->curformat, $utcValue);
            $raw = $utcValue;
        } else {
            $raw = $numValue;
            ...
            $string = gmdate ($this->curformat, mktime($hours, $mins, $secs));
        }
