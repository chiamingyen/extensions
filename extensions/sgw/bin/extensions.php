...
<body>
<div class="border headline">Simple Groupware '.CORE_VERSION_STRING.' Extensions</div>
');
...
if ((empty($_REQUEST["extension"]) and empty($_REQUEST["uninstall"]) and empty($_REQUEST["cfile"])) or !sys_validate_token()) {
  ...
  ");
  setup::out("Downloading extension list ...<br>");
  ...
  if (($xml = @simplexml_load_string($data))) {
	foreach ($xml as $package) {
	  ...
	  if (version_compare(PHP_VERSION, $php_version, "<")) {
		setup::out(sprintf("Setup needs php with at least version %s ! ", $php_version), false);
	  } else if (version_compare(CORE_VERSION_STRING, $sgs_version, "<")) {
		setup::out(sprintf("Setup needs Simple Groupware with at least version %s ! ", $sgs_version), false);
	  } else {
		setup::out("<a href='extensions.php?token=".modify::get_form_token()."&extension=".$package->name."&filename=".$package->filename."'>I n s t a l l</a> ", false);
	  }
	  setup::out($package->title." <a href='#' onclick='return showhide(\"".$id."\")'>Info</a>", false);
	  setup::out("<br><div class='description' style='display:none;' id='".$id."'>".nl2br(trim($package->description))."</div>");
	}
  } else {
	setup::out(sprintf("Connection error: %s [%s]", $url, "HTTP")."<br>".strip_tags($data, "<br><p><h1><center>"));
  }
  
  setup::out("Package from local file system (.tar.gz):<br/>current path: ".str_replace("\\","/",getcwd())."/<br/>");

  $dir = opendir("./");
  while (($file=readdir($dir))) {
    if ($file!="." and $file!=".." and preg_match("|^SimpleGroupware\_.*?.tar\.gz\$|i",$file)) {
	  setup::out("<a href='extensions.php?token=".modify::get_form_token()."&cfile=".$file."'>I n s t a l l</a>&nbsp; ".$file."<br/>");
	}
  }
  closedir($dir);
  setup::out("<form method='POST'><input type='hidden' name='token' value='".modify::get_form_token()."'><input type='text' name='cfile' value='/tmp/SimpleGroupware_SomeExtension_0.x.tar.gz' style='width:300px;'>&nbsp;<input type='submit' class='submit' value='I n s t a l l'><br>");
  ...
  foreach (scandir(SIMPLE_EXT) as $file) {
	...
	setup::out("<a onclick='if (!confirm(\"Really uninstall the module ?\")) return false;' href='extensions.php?token=".modify::get_form_token()."&uninstall=".$package->filename."'>U n i n s t a l l</a> ".$package->title, false);
	setup::out(" <a href='#' onclick='return showhide(\"".$id."\")'>Info</a>", false);
	setup::out(" (installed ".sys_date("m/d/Y", filemtime(SIMPLE_EXT.$file)).")");
	setup::out("<div class='description' style='display:none;' id='".$id."'>".nl2br(trim($package->description))."</div>");
	$can_uninstall = true;
  }
  if ($can_uninstall) setup::out("<b>Note:</b> Uninstall does not delete any data in the database.<br>");
...
if (!empty($_REQUEST["uninstall"])) {
  ...
  setup::out("<br><a href='extensions.php'>C O N T I N U E</a>");
...
if (!empty($_REQUEST["cfile"])) {
  ...
  if (!file_exists($source) or filesize($source)==0) sys_die("Error: file-check [0] ".$source);
  ...
  if (!is_array($file_list) or empty($file_list[0]["filename"])) {
	sys_die("Error: file-check [0b] ".$source);
  }
...
abstract class extensions {
  static function uninstall($filename) {
	...
	setup::out(sprintf("Processing %s ...",basename($source)));
  ...
  static function install($source, $filename) {
	...
	if ($fz = gzopen($source,"r") and $fp = fopen($target,"w")) {
	  ...
	  gzclose($fz);
	  fclose($fp);
	} else sys_die("Error: gzopen [2] ".$source);

	setup::out();
	if (!file_exists($target) or filesize($target)==0 or filesize($target)%10240!=0) {
	  sys_die("Error: file-check [3] Filesize: ".filesize($target)." ".$target);
	}
	setup::out(sprintf("Processing %s ...",basename($target)));
	...
	foreach ($file_list as $file) {
	  ...
	  setup::out(sprintf("Processing %s ...",SIMPLE_EXT.$file["filename"]));
  ...
  function update_modules_list() {
	setup::out(sprintf("<br>Processing %s ...",SIMPLE_EXT."*/modules.txt"));
	$data = "";
	foreach (scandir(SIMPLE_EXT) as $file) {
	...
	setup::out(sprintf("Processing %s ...",SIMPLE_EXT."*/sys_modules.txt"));
	$data = "";
	foreach (scandir(SIMPLE_EXT) as $file) {
	  if ($file[0]=="." or !is_dir(SIMPLE_EXT.$file)) continue;
