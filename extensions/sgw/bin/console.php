...
if ($_REQUEST["console"]=="php") {
  ...
  if ($code!="") {
	if (($data = sql_fetch($code,false)) === false) {
	  ...
	  $content .= sprintf("%s rows affected",$num);
	} else {
	  $content .= "Empty";
	}
  }
...
if ($code!="") {
  $content .= "<br/>&nbsp;".sprintf("%s secs", number_format(microtime(true)-$start, 4));
  $content .= ", ".sprintf("%s M memory usage", number_format(memory_get_peak_usage(true)/1048576, 2));
}
