...
static function ajax_store_vote($folder, $votes) {
  ...
  foreach ($votes as $qid=>$vote_elems) {
	...
	if (isset($votedby[$id])) exit("Already voted. (".$qid.")");
...
static function choices($var, $args, $data) {
  ...
  if (empty($data["qtype"]["data"][0])) {
    if (!$has_choices) return "Thanks for voting!";
    return <<<EOT
	  <input class="surveys submit bold" type="button" value="V o t e !" style="margin:0px;" onclick="
